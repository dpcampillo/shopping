...
public class AppAuthorizationFilter extends BasicAuthenticationFilter{
	...
	protected void doFilterInternal(HttpServletRequest req, HttpServletResponse res, FilterChain chain)
		...
		String header = req.getHeader(SecurityCons.HEADER_AUTHORIZACION_KEY);
        if (header == null || !header.startsWith(SecurityCons.TOKEN_BEARER_PREFIX)) {
	...
	private UsernamePasswordAuthenticationToken getAuthentication(HttpServletRequest request) {
		...
		if (token != null) {
			...
			String user = Jwts.parser().setSigningKey(SecurityCons.SUPER_SECRET_KEY)
					.parseClaimsJws(token.replace(SecurityCons.TOKEN_BEARER_PREFIX, "")).getBody().getSubject();
			if (user != null) {
